macro(add_shaders ARG_TARGET)
  ...
  foreach(SHADER ${ARG_SHADERS})
    ...
    # GLSL
    shaderc_parse(OUT FILE "${SHADER}" OUTPUT "${SHADER_DIRECTORY}/glsl/${SHADER_NAME}.bin" ${T} VARYINGDEF "${SHADER_DIR}/varying.def.sc" LINUX PROFILE 420 ${BASE_OPTIONS})
    list(APPEND SHADER_COMMANDS COMMAND "$<TARGET_FILE:shaderc>" ${OUT})

    # Metal
    shaderc_parse(OUT FILE "${SHADER}" OUTPUT "${SHADER_DIRECTORY}/metal/${SHADER_NAME}.bin" ${T} VARYINGDEF "${SHADER_DIR}/varying.def.sc" OSX ${BASE_OPTIONS} PROFILE metal)
    list(APPEND SHADER_COMMANDS COMMAND "$<TARGET_FILE:shaderc>" ${OUT})

    list(APPEND SHADER_SOURCE_FILES "${SHADER}")
  endforeach()
